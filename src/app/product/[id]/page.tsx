      ...
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl p-6 md:p-10 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-8 lg:gap-16">
          ...
          <div className="w-full md:w-1/2">
            ...
            <div className="flex gap-3">
              {product.images.map((img, i) => (
                <div key={i} className="w-20 h-20 rounded-lg bg-gray-50 border border-gray-200 p-1 cursor-pointer overflow-hidden">
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-center">
            <Link href={`/shop?q=${encodeURIComponent(product.seller.storeName)}`} className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2 hover:text-[#1e3b8a] transition-colors">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="text-3xl font-black text-[#1e3b8a]">{product.basePrice.toLocaleString()} {t.EGP}</div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {product.tags.map(tag => (
                <span key={tag.id} className="text-xs font-semibold text-gray-500 border border-gray-200 bg-white px-3 py-1 rounded-full">
                  #{tag.name}
                </span>
              ))}
            </div>
