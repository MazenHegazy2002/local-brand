...
export default async function SellerDashboard() {
  ...
  // Resolve the live, escrow-aware available balance instead of trusting
  // the legacy sellerProfile.balance column.
  const earnings = sellerProfile
    ? await computeSellerEarnings(sellerProfile.id)
    : {
        available: 0,
        held: 0,
        totalEarned: 0,
  ...
  const outOfStock = sellerProfile.products.filter(
    p => p.variants.reduce((acc, v) => acc + v.stockCount, 0) === 0
  ).length;
      ...
      <div className="container py-12">
        <div className="flex justify-between items-end mb-12">
          <div>
            <h1 className="text-4xl font-serif font-bold text-white mb-2">
              {sellerProfile.storeName} <span className="text-[hsl(var(--accent))]">Dashboard</span>
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="glass p-8 rounded-3xl border border-white/5">
            <span className="text-white/40 text-xs font-bold uppercase tracking-widest block mb-4">
              Wallet Balance
            </span>
            <span className="text-3xl font-serif font-bold text-white block mb-2">
              {earnings.available.toLocaleString()} EGP
            </span>
            <span className="text-white/30 text-xs block mb-2">
              {earnings.held > 0
                ? `${earnings.held.toLocaleString()} EGP in 14-day escrow`
                : 'Ready for withdrawal'}
            </span>
            <Link
              href="/seller-hub?tab=wallet"
              className="text-[hsl(var(--accent))] text-xs cursor-pointer hover:underline"
            >
              Request Payout
            </Link>
          </div>
          <div className="glass p-8 rounded-3xl border border-white/5">
            ...
            <span className="text-white/30 text-xs">
              {outOfStock > 0 ? `${outOfStock} Out of Stock` : 'All items in stock'}
            </span>
          </div>
          <div className="glass p-8 rounded-3xl border border-white/5">
            <span className="text-white/40 text-xs font-bold uppercase tracking-widest block mb-4">
            ...
            <span className="text-3xl font-serif font-bold text-white block mb-2">
              {(sellerProfile.commissionRate * 100).toFixed(0)}%
            </span>
        ...
        <div className="glass rounded-[2.5rem] border border-white/5 p-10">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-serif font-bold text-white">
              Current <span className="text-white/20">Inventory</span>
            </h2>
            <input
              type="text"
              placeholder="Search products..."
              className="bg-transparent border-b border-white/20 text-white p-2 outline-none focus:border-[hsl(var(--accent))]"
            />
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-6 text-white/60">
                        <span className={stock === 0 ? 'text-red-400 font-bold' : ''}>
                          {stock} Units
                        </span>
                      </td>
                      <td className="py-6">
