...
export async function POST(req: Request) {
  try {
    ...
    const isRefund =
      message.toLowerCase().includes('refund') || message.toLowerCase().includes('return');
    const isShipping =
    ...
    if (isHello) {
      ...
      if (orderId) {
        ...
        if (order) {
          ...
          response += order.items
            .map(item => `• ${item.variant.product.title} x${item.quantity}`)
            .join('\n');
        ...
        if (orders.length > 0) {
          ...
          response += orders
            .map(o => `• #${o.id.slice(0, 8)} - ${o.status} (${o.totalAmount} EGP)`)
            .join('\n');
      ...
      if (productId) {
        ...
        response =
          "I'd be happy to help you find a product! What are you looking for? You can browse our shop at /shop";
      }
    } else if (isShipping) {
      response = '🚚 For shipping inquiries:\n\n';
      response += '• Standard delivery: 3-7 business days\n';
      response += '• Express delivery: 1-2 business days\n';
      response += '• Free shipping on orders over 1000 EGP\n\n';
      ...
      if (keywords.some(k => message.toLowerCase().includes(k))) {
        ...
        response =
          'I understand you\'re looking for help with "' + message.slice(0, 50) + '..."\n\n';
