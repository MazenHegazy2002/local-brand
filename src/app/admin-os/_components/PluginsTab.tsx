...
export default function PluginsTab() {
  ...
  const [activeCat, setActiveCat] = useState<string>('PAYMENT');
  ...
  const visiblePlugins = useMemo(() => {
    ...
    if (!cat) return [];
    if (!search.trim()) return cat.plugins;
    const q = search.toLowerCase();
    return cat.plugins.filter(
      p =>
          ...
          {categories.map(cat => (
            ...
            >
              ...
              <span className="plugins-rail-count">
                {cat.plugins.filter(p => p.isEnabled).length}/{cat.plugins.length}
              </span>
            </button>
          ))}
        </aside>
              ...
              >
                ...
                <div className="plugin-card-foot">
                  {plugin.envConfigured && !plugin.installed && (
                    <span className="plugin-card-tag plugin-card-tag-info">Configured via env</span>
                  )}
                  {plugin.installed && (
                    <span
