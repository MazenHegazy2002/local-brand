    ...
    return (
      <main className="min-h-screen bg-[hsl(var(--background))]">
        ...
        <div className="container py-12 text-center text-white">
          ...
          <Link href="/dashboard" className="btn btn-accent mt-6">Return to Dashboard</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[hsl(var(--background))]">
      ...
      <div className="container py-12">
        <Link href="/dashboard" className="text-white/40 hover:text-white mb-8 inline-block">&larr; Back to Dashboard</Link>
        
        <div className="flex justify-between items-end mb-8">
          <div>
            <h1 className="text-4xl font-serif font-bold text-white mb-2">Order <span className="text-[hsl(var(--accent))]">#{order.id.split('-')[0]}</span></h1>
            <p className="text-white/40 text-sm tracking-widest uppercase font-bold">Placed on {new Date(order.createdAt).toLocaleDateString()}</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-serif text-white">{order.totalAmount.toLocaleString()} EGP</p>
            <p className="text-[hsl(var(--accent))] text-sm font-bold uppercase tracking-widest">{order.paymentStatus}</p>
          </div>
        </div>
        ...
        <div className="glass p-8 rounded-3xl border border-white/5 mb-8">
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <div key={step} className="flex flex-row md:flex-col items-center gap-4 md:gap-0">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border-4 border-[hsl(var(--background))] z-10 ${isActive ? 'bg-[hsl(var(--accent))] text-[hsl(var(--background))]' : 'bg-white/20 text-transparent'}`}>
                      {isActive && '✓'}
                    </div>
                    <span className={`md:mt-4 text-xs font-bold tracking-widest uppercase ${isActive ? 'text-white' : 'text-white/40'}`}>
                      {step.replace('_', ' ')}
                    </span>
                  </div>
                );
          ...
          {order.shipments.length > 0 && (
            <div className="mt-8 pt-8 border-t border-white/5">
              <h3 className="text-sm font-bold text-white uppercase tracking-widest mb-4">Shipment Details</h3>
              {order.shipments.map(shipment => (
                <div key={shipment.id} className="flex gap-4 items-center">
                  <div className="bg-white/5 p-4 rounded-xl">
                    ...
                    <span className="text-white font-bold">{shipment.courier || 'Standard Shipping'}</span>
                  </div>
                  <div className="bg-white/5 p-4 rounded-xl">
                    <span className="block text-white/40 text-xs mb-1">Tracking Number</span>
                    <span className="text-[hsl(var(--accent))] font-bold tracking-widest">{shipment.trackingNumber || 'Pending'}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid gap-4">
          {order.items.map(item => {
            ...
            return (
              <div key={item.id} className="glass p-6 rounded-2xl border border-white/5 flex gap-6 items-center">
                <div className="w-24 h-24 relative rounded-xl overflow-hidden bg-white/5 hidden sm:block">
                  <img src={primaryImage} alt={item.productTitleSnapshot} className="object-cover w-full h-full" />
                </div>
                <div className="flex-grow">
                  ...
                  <div className="flex gap-4">
                    ...
                    <span className="text-white/60 text-sm">Price: {item.priceAtPurchase.toLocaleString()} EGP</span>
                  </div>
                </div>
                <div className="text-right">
