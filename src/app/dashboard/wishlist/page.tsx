...
export default function WishlistPage() {
  ...
  const variantIdFor = (item: WishlistItem) =>
    item.product.variants?.[0]?.id || String(item.productId);
  ...
  return (
    <div className="db">
      <div className="sidebar">
        ...
        <Link href="/dashboard/wishlist" className="nav-item active">
          Wishlist
        </Link>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      className="font-medium text-sm hover:text-[#534AB7] truncate block"
                    >
                      {item.product.title}
                    </Link>
                    <p className="text-xs text-slate-500 mb-2">{item.product.category?.name}</p>
                    <p className="font-bold text-[#534AB7]">
                      {item.product.basePrice?.toLocaleString()} EGP
                    </p>
                  </div>
                </div>
