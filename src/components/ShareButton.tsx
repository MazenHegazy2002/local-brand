    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl border border-gray-100 shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
          <a
            href={`https://wa.me/?text=${encodeURIComponent(`Check out ${productName} on Brandy! ${typeof window !== 'undefined' ? window.location.origin : ''}/product/${productId}`)}`}
            target="_blank"
          ...
          <a
            href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`${typeof window !== 'undefined' ? window.location.origin : ''}/product/${productId}`)}`}
            target="_blank"
            rel="noopener noreferrer"
